...
`capture/frida-server.exe`. That binary is the Windows build, because it is
...
Check a ledger without leaving this repository:

```sh
.venv-capture/bin/python capture/decode_dtls_ledger.py path/to/ledger.bin --summary-only
```

It reports record counts, decode errors and the Carrier channel histogram. Decoding the
game protocol itself belongs to the research project, not here.

The same files stay on the machine under `capture/captures/web/<session-id>/`.
...
- **Video state is `ERROR` but the capture finished.** The ZIP is still valid and keeps
  the ledger. Read `capture/captures/web/<session-id>/video.log`.
...
```sh
node capture/web/test.ts
.venv-capture/bin/python -m unittest discover -s capture/web -p 'test_*.py'
.venv-capture/bin/python -m unittest discover -s capture -p 'test_*.py'
node capture/test_agent.js
```

The web test uses a fake collector subprocess, never the game. It covers concurrent
start and stop, cancellation during STARTING, the shared snapshot, the wait for the
child to exit, the ZIP contents, traversal routes, Origin rejection, a failed spawn
and SIGTERM shutdown. Node runs TypeScript without type-checking it.

## Security

The page is loopback only, with no authentication, no TLS and no multi-user isolation.
Any local user, or anyone with a tunnel to that port, controls the session and reads
the aggregate metadata. Do not expose the port or put it behind a reverse proxy.

The ledger holds raw game traffic and the host keeps HTTPS bodies, headers and a TLS
keylog under `capture/captures/`. Those stay on the machine: the ZIP carries only
metadata, the ledger and the video. Treat both as private.

## Where this comes from

Extracted from the New World capture research project, which keeps the protocol
documentation, the offline ledger decoders and the live map. This repository holds only
what is needed to record a session: the Frida collector, the injected agent, the local
web control and the archiver. Analysing a `ledger.bin` happens in that project.

Licensed under AGPL-3.0, see [LICENSE](LICENSE). Frida and gpu-screen-recorder are
installed from their own upstream releases and keep their own licences; neither is
vendored here.
