...
.venv-capture/bin/python -m pip install --quiet -r capture/requirements.txt
...
if [[ ! -f capture/frida-server.exe ]]; then
    work=$(mktemp -d)
    ...
    unxz "$work/$asset"
    mv "$work/${asset%.xz}" capture/frida-server.exe
    chmod 600 capture/frida-server.exe
fi
...
echo 'Setup complete. Start a session with: bash capture/web/run.sh'
